...
var render = function() {
    ...
    theta[4]+=speedrightarm;
    initNodes(4);
    theta[2]+=speedleftarm;
    initNodes(2);
    theta[8]+=speedrightleg;
    initNodes(8);
    theta[6]+=speedleftleg;
    initNodes(6);


    //Hægri handleggur númer númer 4 í lista range(-90,-270)
    //Byrjar í -30 og fær -5 breyta um átt ef -90
    if(-90<=theta[4] || theta[4]<=-270){
        speedrightarm=-speedrightarm;

    }
    
    
    //Vinstri handleggur númer 2 í lista range(90,270)

    if(270<=theta[2] || theta[2]<=90){
        speedleftarm=-speedleftarm;
